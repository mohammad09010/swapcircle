...
router.get("/", async (req, res, next) => {
  try {
    ...
    // Recommendations are personalised when logged in,
    // otherwise we fall back to popular items.
    const recommendations = req.currentUser
      ? await recommendForUser(req.currentUser.user_id, 4)
      : [];
